...
{
    public class ReportCashBoxController : Controller
    {
        private readonly HotelAlkheerDB _db;

        public ReportCashBoxController(HotelAlkheerDB context)
        {
            _db = context;
        }
        public IActionResult Index()
        {
           // 
            ...
            List<AccountTable> accM = new List<AccountTable>();
            accM.Add(new AccountTable
            {
                Id = -1,
                Name = "اختر صندوق"
            });

            var mm= _db.AccountTables
              .Where(x => x.IdGroup == Static_Group_Accounts.Boxs)
              .Select(xx => new AccountTable
              {
                  Id = xx.Id,
                  Name = xx.Name
              })
        ...
        ViewData["IdAccount"] = new SelectList(accM, "Id", "Name", 0);
