    ...
    if (validation == true) {
        ...
        $.post(url, myModel, function (response) {
            ...
            if (response.status == 0) {
                alert(response.mess + response.id + "id My : " + response.idmy);
        ...
        var dt = $('#table13')
            .DataTable({
                ...
                "columns": [
                    ...
                    //        return "<a class='btn btn-danger btn-sm' style='margin-left:5px' onclick=Delete(" + row + ")><i class='fa fa-trash'></i> Delete</a>";
...

                    {
                        ...
                        "render": function (data, type, row, meta) {
                            ...
                            //'<button class="btn btn-success" onclick="selectCustomerClick(' + JSON.stringify(row) + ')">' + 'Add' + '</button>';

                            //"<a class='btn btn-default btn-sm' onclick='selectCustomerClick(" + row + ")'><i class='fa fa-pencil'></i> Edit</a>";

                            //return '<button  data-id="' + row.name + '"   data-privateNote="' + row.privateNote + '"  data-toggle="' + row.idcumtomerAll + '" onclick="selectCustomerClick('+row+')">تحديد</button>';
...
function selectCustomerClick(row) {
    ...
    $("#idcityHid").val(row.area.idCity);
    $("#idareaHid").val(row.area.id);

    $("#idcountryHid").val(row.area.idCountry);

    alert(row.area.idCountry);

    $("#country").val(row.area.idCountry).change();

  

    $("#TypeProof").val(row.typeProof).change();
